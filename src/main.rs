...
struct Opt {
    /// Calculate an expression directly
...
fn main() {
    ...
    match opt {
        Opt { expression: Some(expr), .. } => {
            // 处理单个表达式
            let result = calculate_expression(&expr);
            println!("Result: {}", result);
        },
        Opt { path: Some(file_path), .. } => {
            // 从文件中读取并处理表达式
...
fn interactive_mode() {
    println!("{}","Entering interactive mode. Type 'exit' to quit.".blue().bold());
    // 这里应该是你的交互模式逻辑
    // 例如：读取用户输入，计算表达式，显示结果
    // 在这个简单的例子中，我们将直接退出
    println!("{}","Exiting interactive mode.".blue().bold());
}